...
# adapted from https://github.com/AstarLight/CPS-OCR-Engine/blob/master/ocr/gen_printed_char.py
...
def args_parse():
    ...
    parser.add_argument('--font_dir', dest='font_dir',
                        default=None, required=True,
                        help='font dir to produce images')
    parser.add_argument('--dict_path', dest='dict_path',
                        default=None, required=True,
                        help='dict path from which the images are generated')
    ...
    parser.add_argument('--dry_run', dest='dry_run',
                        default=False, required=False,
                        help='just generate a single random sample to view the effect', action='store_true')   
...
python gen_printed_char.py --out_dir ./dataset \
    ...
    if dry_run:
        i = random.randint(0, len(label_dict)-1)
        label_dict = {i: label_dict[i]} 
    
    char_list=[]  # 汉字列表
    value_list=[] # label列表
    prog_bar = tqdm(label_dict.items(), desc='读取字表')
    for (value,chars) in prog_bar:
        prog_bar.set_postfix(chars=chars)
